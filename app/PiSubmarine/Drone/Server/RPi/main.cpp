    ...
    namespace
    {
        struct HardwareConfig
        {
            ...
            Battery::Motherboard::Config Battery{
                .DesignCapacity = Max17261::MicroAmpereHours{5'000'000},
                .ChargeTerminationCurrent = Max17261::MicroAmperes{200'000},
...
int main(const int argc, char** argv)
{
    ...
    try
    {
        ...
        app.add_option("--client-ca", clientCertificateAuthorityPath, "PEM client certificate authority file")->required();
        app.add_option("--control-address", controlAddress, "Control UDP bind address")->default_val(controlAddress);
        app.add_option("--telemetry-address", telemetryAddress, "Telemetry UDP bind address")->default_val(telemetryAddress);
        app.add_option("--video-resource-id", videoResourceId, "Lease resource id used for video streaming")->default_val(videoResourceId);
        app.add_option("--video-source", videoSourceDescription, "Optional explicit GStreamer source element description. Leave empty for autodetect.");
        app.add_option("--startup-video-endpoint", startupVideoEndpoint, "Optional host:port RTP endpoint to subscribe immediately at startup.");
        app.add_flag("--startup-video-enable", startupVideoEnable, "Enable video streaming target at startup using low-latency profile and autofocus.");
        app.add_option("--tick-period-ms", tickPeriodMilliseconds, "Tick period in milliseconds")->default_val(tickPeriodMilliseconds);
        app.add_option("--receive-queue-capacity", config.ReceiveQueueCapacity, "Per-socket receive queue capacity")->default_val(config.ReceiveQueueCapacity);
        app.add_option("--max-datagram-size", config.MaxDatagramSize, "Maximum UDP datagram size in bytes")->default_val(config.MaxDatagramSize);
        ...
        app.add_option("--lamps-ballast-spi-device", hardware.LampsAndBallastSpiDevice, "SPI device for the lamps and ballast DRV8908")
        ...
        app.add_option("--battery-store", hardware.BatteryStorePath, "Persistent file path for MAX17261 learning parameters")
            ->default_val(hardware.BatteryStorePath.string());
        app.add_option("--thrusters-n-sleep-pin", hardware.ThrustersNSleepPin, "GPIO line connected to thrusters DRV8908 nSLEEP")
            ->default_val(hardware.ThrustersNSleepPin);
        app.add_option("--thrusters-n-fault-pin", hardware.ThrustersNFaultPin, "GPIO line connected to thrusters DRV8908 nFAULT")
            ->default_val(hardware.ThrustersNFaultPin);
        app.add_option("--lamps-ballast-n-sleep-pin", hardware.LampsAndBallastNSleepPin, "GPIO line connected to lamps/ballast DRV8908 nSLEEP")
            ->default_val(hardware.LampsAndBallastNSleepPin);
        app.add_option("--lamps-ballast-n-fault-pin", hardware.LampsAndBallastNFaultPin, "GPIO line connected to lamps/ballast DRV8908 nFAULT")
            ->default_val(hardware.LampsAndBallastNFaultPin);
        app.add_option("--spi-speed-hz", hardware.SpiSpeed, "SPI bus speed for both DRV8908 devices")->default_val(hardware.SpiSpeed);
        app.add_option("--battery-design-capacity-uah", batteryDesignCapacityUah, "MAX17261 design capacity in microampere-hours")
            ->default_val(batteryDesignCapacityUah);
        app.add_option("--battery-charge-termination-ua", batteryChargeTerminationUa, "MAX17261 charge termination current in microamperes")
            ->default_val(batteryChargeTerminationUa);
        app.add_option("--battery-empty-voltage-uv", batteryEmptyVoltageUv, "MAX17261 battery empty voltage in microvolts")
            ->default_val(batteryEmptyVoltageUv);
        app.add_flag("--battery-force-gauge-reset", hardware.Battery.ForceGaugeReset, "Force MAX17261 gauge reset during startup.");
        ...
        {
            SPDLOG_LOGGER_ERROR(logger, "Battery provider initialization failed: {}", ToString(batteryInitResult.error()));
            return 1;
        }
